...
if LINUX:
    try:
        subprocess.check_call(['dpkg', '--version'], stdout=PIPE, stderr=PIPE)  # nosec
        LINUX_DEB = True
    except Exception:  # pylint: disable=broad-except
        LINUX_DEB = False

    try:
        subprocess.check_call(['rpm', '--version'], stdout=PIPE, stderr=PIPE)  # nosec
        LINUX_RPM = True
    except Exception:  # pylint: disable=broad-except
        LINUX_RPM = False

    try:
        subprocess.check_call(['dnf', '--version'], stdout=PIPE, stderr=PIPE)  # nosec
        LINUX_DNF = True
    except Exception:  # pylint: disable=broad-except
        LINUX_DNF = False
...
AD_CACHE: 'typing_extensions.Final[str]' = os.path.join(CONF_PATH, 'ad_cache')
