...
NUCLEUS_LOGIN_TEXT_CONTAINER = utils.TextContainer(
    title='Sign In',
    info_frame_text=(
        utils.Span(
            'Log into Anaconda Nucleus to backup your environment and access '  # pylint: disable=implicit-str-concat
            'more data science tools. If you don’t have an account, click '
...
class NucleusLoginPage(BaseLoginPage):  # pylint: disable=missing-class-docstring
    ...
    def __init__(self, parent: typing.Optional[QWidget] = None) -> None:
        ...
        self.text_login.setValidator(utils.EMAIL_RE_VALIDATOR)
    ...
    def login(self):
        ...
        worker: workers.TaskWorker = self.api.login.worker(
            username=text_login,
    ...
    def __process_login_response(self, result: workers.TaskResult) -> None:
    ...
    def update_links(self):
        ...
        forgot_url = attribution.POOL.settings.inject_url_parameters(
            url_utils.join(base_url, 'forgot-password'),
            utm_medium='connect-nucleus',
            utm_content='password-reset',
        )
        sign_up_url = attribution.POOL.settings.inject_url_parameters(
            url_utils.join(base_url, 'sign-up'),
            utm_medium='connect-nucleus',
            utm_content='signup',
        )

        forgot_links_updated_text = self.forgot_links.text().format(password_url=forgot_url)
        info_updated_text = self.label_information.text().format(sign_up_url)
