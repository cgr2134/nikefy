...
class _AnacondaAPI(QObject):  # pylint: disable=too-many-instance-attributes,too-many-public-methods
    ...
    def _process_conda_info(info):
        ...
        if (not isinstance(info, typing.Mapping)) or ('envs_dirs' not in info):
            ...
            if actions:
                ...
                for pkg in unlinked:
                    if pkg in navigator_dependencies:
                        u_pkg_ver = LooseVersion(unlinked[pkg][1])
                        l_pkg = linked.get(pkg)
                        l_pkg_ver = LooseVersion(linked[pkg][1]) if l_pkg else None
                ...
                for down_dep, down_dep_version in downgraded_deps.items():
                    ...
                    if nav_dep_version:
                        nav_dep_version = LooseVersion(nav_dep_version)
