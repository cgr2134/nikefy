...
Config = namedtuple(
    'Config',
    ['client_id', 'port', 'authorize_url', 'token_url', 'redirect_uri', 'server', 'verify_ssl']
)
...
class RequestHandler(BaseHTTPRequestHandler):
    ...
    def process_code(self, code):
        ...
        r = requests.post(
            token_url, data, verify=self.config.verify_ssl
        )
...
class WebServer:
    def __init__(self, client_id, openid_configuration_url, port=None, verify_ssl=None):
        ...
        openid_conf = requests.get(openid_configuration_url, verify=self.verify_ssl).json()
