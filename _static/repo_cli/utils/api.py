...
class RepoApi:
    ...
    def create_access_token(self):
        ...
        resp = requests.post(self.urls['account_tokens'], data=json.dumps(data), headers=self.bearer_headers, verify=self.verify_ssl)
    ...
    def get_xauth_token(self, force_scopes):
        ...
        token_resp = requests.get(self.urls['account_tokens'], headers=self.bearer_headers, verify=self.verify_ssl)
        ...
        if force_scopes:
            ...
            logger.debug(f'[XAUTH_TOKEN] Deleteing token because force_scopes option {token_id}')
            token_resp = requests.delete(token_url, headers=self.bearer_headers, verify=self.verify_ssl)
            ...
            token_resp = requests.put(token_url, data="{}", headers=self.bearer_headers, verify=self.verify_ssl)
            if token_resp.status_code != 200:
    ...
    def get_authorize_url(self) -> 'URL':
        s = requests.Session()
        logger.debug(f'[LOGIN] Getting authorize endpoint')
        resp = s.get(self.urls['authorize'], verify=self.verify_ssl, allow_redirects=False)
    ...
    def login(self, username, password) -> requests.Response:
        ...
        resp = s.post(self.urls['login'], data=json.dumps(data), headers={'Content-Type': 'application/json'}, verify=self.verify_ssl)
    ...
    def get_default_channel(self):
        ...
        return self._manage_response(response, 'getting account details')['default_channel_name']
    ...
    def upload_file(self, filepath, channel, package_type):
        ...
        response = requests.post(url, files=multipart_form_data, headers=self.xauth_headers, verify=self.verify_ssl)
        return response
    ...
    def create_channel(self, channel):
        ...
        return self._manage_response(response, f'creating channel {_channel}', success_codes=[201])
    ...
    def remove_channel(self, channel):
        ...
        response = requests.delete(url, headers=self.get_xauth_headers({'Content-Type': 'application/json'}), verify=self.verify_ssl)
    ...
    def update_channel(self, channel, success_message=None, **data):
        ...
        logger.debug(f'Using token {self._access_token}')
        response = requests.put(url, json=data, headers=self.get_xauth_headers({'Content-Type': 'application/json'}), verify=self.verify_ssl)
    ...
    def get_channel(self, channel):
        ...
        url = self._get_channel_url(channel)
        response = requests.get(url, headers=self.get_xauth_headers({'Content-Type': 'application/json'}), verify=self.verify_ssl)
    ...
    def list_user_channels(self):
        ...
        response = requests.get(self.urls['user_channels'],
                                headers=self.get_xauth_headers({'Content-Type': 'application/json'}), verify=self.verify_ssl)
        return self._manage_response(response, f'getting user channels')

    def list_channels(self):
        logger.debug(f'Getting channels from {self.base_url}')
        response = requests.get(self.urls['channels'],
                                headers=self.get_xauth_headers({'Content-Type': 'application/json'}), verify=self.verify_ssl)
        return self._manage_response(response, f'getting channels')
    ...
    def create_mirror(self, channel, source_root, name, mode, filters, type_, cron, run_now, proxy):
        ...
        resp = requests.post(url, data=json.dumps(mirror_details), headers=self.get_xauth_headers({'Content-Type': 'application/json'}), verify=self.verify_ssl)
        return self._manage_response(resp, f'Creating mirror {name} on channel {channel}', success_codes=[201], auth_fail_codes=[401])

    def update_mirror(self, mirror_id, channel, source_root, name, mode, filters, type_, cron, run_now, proxy):
        ...
        resp = requests.put(url, data=json.dumps(mirror_details), headers=self.get_xauth_headers({'Content-Type': 'application/json'}), verify=self.verify_ssl)
        return self._manage_response(resp, f'Updating mirror {name} on channel {channel}', success_codes=[202], auth_fail_codes=[401])
    ...
    def get_mirror(self, channel, mirror_name):
        ...
        resp = requests.get(url, headers=self.xauth_headers, verify=self.verify_ssl)
        return self._manage_response(resp, f'Getting mirror {mirror_name} on channel {channel}')
    ...
    def delete_mirror(self, channel, mirror_id, mirror_name):
        ...
        return self._manage_response(resp, f'Mirror {mirror_name} was deleted on channel {channel}',
                                     success_codes=[204])

    # TOKEN RELATED URLS
    def _manage_response(self, response, action='', success_codes=None, auth_fail_codes=None):
    ...
    def get_token_info(self):
        response = requests.get(self.urls['token_info'], headers=self.xauth_headers, verify=self.verify_ssl)
    ...
    def get_user_tokens(self):
        response = requests.get(self.urls['account_tokens'], headers=self.bearer_headers, verify=self.verify_ssl)
    ...
    def remove_user_token(self, token):
        ...
        response = requests.delete(url, headers=self.bearer_headers, verify=self.verify_ssl)
        return self._manage_response(response, 'removing user token', success_codes=[204])
    ...
    def create_user_token(self, name, expiration, scopes=None, resources=None):
        ...
        response = requests.post(self.urls['account_tokens'], data=json.dumps(data),
                                 headers=self.bearer_headers, verify=self.verify_ssl)
        return self._manage_response(response, "creating user token", success_codes=[200])

    def get_scopes(self):
        response = requests.get(self.urls['scopes'], headers=self.bearer_headers, verify=self.verify_ssl)
    ...
    def channel_artifacts_bulk_actions(self, channel, action, artifacts, target_channel=None):
        ...
        return self._manage_response(resp, "%s articfacts" % action, success_codes=[202],
    ...
    def get_channel_artifacts(self, channel, offset=None, limit=None):
        ...
        if query:
            url = join(self._get_channel_url(channel), 'artifacts?{}'.format('&'.join(query)))
        else:
    ...
    def _parse_conda_file(self, file_data, version, filename, return_raw = False):
        meta = file_data["metadata"].get("repodata_record.json") or file_data['metadata']['index.json']
    ...
    def get_artifacts(self, query, limit=50, offset=0, sort="-download_count"):
        url = "%s?q=%s&limit=%s&offset=%s&sort=%s" %(self.urls['artifacts'], query, limit, offset, sort)
        response = requests.get(url, headers=self.xauth_headers, verify=self.verify_ssl)
        return self._manage_response(response, "searching artifacts")

    def get_artifact_files(self, channel, package, family, limit = 100):
        url =  self.urls['channels'] + "/%s/artifacts/%s/%s/files?limit=%s" % (channel, family, package, limit)
        response = requests.get(url, headers=self.xauth_headers, verify=self.verify_ssl)
        return self._manage_response(response, "getting artifact files")

    def get_sbom_download_link(self, channel, ckey = ""):
        return "%s/%s/%s?sbom=download" % (self.urls['repo'], channel, ckey)
    ...
    def get_cve_files(self, cve_id, offset=0, limit=50):
        url = "%s/%s/files?offset=%s&limit=%s" % (self.urls['cves'], cve_id, offset, limit)
        response = requests.get(url, headers=self.xauth_headers, verify=self.verify_ssl)
    ...
    def _validate_channel_name(self, name: str):
        if not re.match(r"^[a-z][a-z0-9_-]*$", name):
            ...
            error_message = f"Channel name contains invalid sequence {', '.join(invalid_letters)}"
